...
**左侧 Tabs**(配合 `#/dashboard`、`#/detail` hash 路由):

1. **数据大屏** —— 纯数据屏:16 个关键数字(文件/函数/代码行/注释占比/平均与最大圈复杂度/平均与最大认知复杂度/最大嵌套/平均函数行数/Halstead 难度/违规/标记),以及图表 —— 有序分布用条形图,占比用环形图。**点任意数字或扇区**会打开下钻抽屉,列出对应的函数/文件;**点列表项**跳到该文件并高亮。ECharts 按需懒加载。
2. **文件详情** —— 左侧文件列表 + 右侧源码预览;点击函数名定位并高亮对应行。

页面正常滚动(不再强制一屏),响应式布局、表格可排序、一键导出 JSON。
